...
def sky130_fd_pr__npn_05v5(
    ...
    # ------------------------------------------------------------------ #
    e_diff = c.add_ref(
        gf.components.rectangle(size=(ew, el), layer=LAYER.diffdrawing)
    )
    ...
    e_licon = c.add_ref(licon_array(width=ew, height=el))
    ...
    tap_outer = gf.components.rectangle(size=(b_outer_w, b_outer_h), layer=LAYER.tapdrawing)
    tap_inner = gf.components.rectangle(size=(b_inner_w, b_inner_h), layer=LAYER.tapdrawing)

    tap_outer_ref = gf.Component()
    tap_outer_ref.add_ref(tap_outer)
    tap_inner_placed = tap_outer_ref.add_ref(tap_inner)
    tap_inner_placed.move((sp + brw - (sp + brw - b_inner_x0 + b_origin_x), sp + brw - (sp + brw - b_inner_y0 + b_origin_y)))
    # Simpler: outer at (b_origin_x, b_origin_y), inner at (b_inner_x0, b_inner_y0)
    # Use gdsfactory boolean directly
    tap_outer_comp = gf.components.rectangle(size=(b_outer_w, b_outer_h), layer=LAYER.tapdrawing)
    tap_inner_comp = gf.components.rectangle(size=(b_inner_w, b_inner_h), layer=LAYER.tapdrawing)
    ...
    base_ring = gf.boolean(A=outer_r, B=inner_r, operation="not", layer=LAYER.tapdrawing)
    ...
    psdm_outer_comp = gf.components.rectangle(size=(psdm_outer_w, psdm_outer_h), layer=LAYER.psdmdrawing)
    psdm_inner_comp = gf.components.rectangle(size=(psdm_inner_w, psdm_inner_h), layer=LAYER.psdmdrawing)
    ...
    li_outer_c = gf.components.rectangle(size=(li_outer_w, li_outer_h), layer=LAYER.li1drawing)
    li_inner_c = gf.components.rectangle(size=(li_inner_w, li_inner_h), layer=LAYER.li1drawing)
    ...
    # Place below device
    coll_li = c.add_ref(
        gf.components.rectangle(size=(coll_li_w, max(coll_li_h, 0.17)), layer=LAYER.li1drawing)
    )
...
def sky130_fd_pr__pnp_05v5(
    ...
    # Li1 pad over emitter
    e_li = c.add_ref(
        gf.components.rectangle(size=(ew, el), layer=LAYER.li1drawing)
    )
    c.add_port(
        name="EMITTER",
        center=(ew / 2, el / 2),
    ...
    base_nsdm_ring = gf.boolean(A=no_r, B=ni_r, operation="not", layer=LAYER.nsdmdrawing)
    c.add_ref(base_nsdm_ring).move((b_origin_x - sdm_enc, b_origin_y - sdm_enc))

    # Base licons (4 sides)
    c.add_ref(licon_array(width=brw, height=b_inner_h)).move((b_origin_x, b_origin_y + brw))
    c.add_ref(licon_array(width=brw, height=b_inner_h)).move((b_origin_x + brw + b_inner_w, b_origin_y + brw))
    c.add_ref(licon_array(width=b_outer_w, height=brw)).move((b_origin_x, b_origin_y))
    c.add_ref(licon_array(width=b_outer_w, height=brw)).move((b_origin_x, b_origin_y + brw + b_inner_h))
    ...
    ctap_o = gf.components.rectangle(size=(c_outer_w, c_outer_h), layer=LAYER.tapdrawing)
    ctap_i = gf.components.rectangle(size=(c_inner_w, c_inner_h), layer=LAYER.tapdrawing)
    ctap_parent = gf.Component()
    cto_r = ctap_parent.add_ref(ctap_o)
    cti_r = ctap_parent.add_ref(ctap_i)
    cti_r.move((c_rw, c_rw))
    coll_tap_ring = gf.boolean(A=cto_r, B=cti_r, operation="not", layer=LAYER.tapdrawing)
    ...
    coll_psdm_ring = gf.boolean(A=cpso_r, B=cpsi_r, operation="not", layer=LAYER.psdmdrawing)
    c.add_ref(coll_psdm_ring).move((c_origin_x - sdm_enc, c_origin_y - sdm_enc))

    # Collector licons
    c.add_ref(licon_array(width=c_rw, height=c_inner_h)).move((c_origin_x, c_origin_y + c_rw))
    c.add_ref(licon_array(width=c_rw, height=c_inner_h)).move((c_origin_x + c_rw + c_inner_w, c_origin_y + c_rw))
    c.add_ref(licon_array(width=c_outer_w, height=c_rw)).move((c_origin_x, c_origin_y))
    c.add_ref(licon_array(width=c_outer_w, height=c_rw)).move((c_origin_x, c_origin_y + c_rw + c_inner_h))
