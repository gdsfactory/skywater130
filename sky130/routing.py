...
def _mark_layer_obstacles(
    ...
    for poly in polygons[layer_name]:
        ...
        xmin_idx = int(
            np.clip(np.floor((xmin_um - x_vals.min()) / resolution), 0, len(x_vals) - 1)
        )
        xmax_idx = int(
            np.clip(np.ceil((xmax_um - x_vals.min()) / resolution), 0, len(x_vals) - 1)
        )
        ymin_idx = int(
            np.clip(np.floor((ymin_um - y_vals.min()) / resolution), 0, len(y_vals) - 1)
        )
        ymax_idx = int(
...
def _simplify_manhattan_path(
    waypoints: list[list[float]], min_segment_length: float = 0.5
) -> list[list[float]]:
    ...
    while changed and len(final) > 3:
        ...
        if len(final) > 3:
            ...
            seg1_len = max(
                abs(final[1][0] - final[0][0]), abs(final[1][1] - final[0][1])
            )
            seg2_len = max(
                abs(final[2][0] - final[1][0]), abs(final[2][1] - final[1][1])
            )
        ...
        if len(final) > 3 and not changed:
            seg_m1_len = max(
                abs(final[-1][0] - final[-2][0]), abs(final[-1][1] - final[-2][1])
            )
            seg_m2_len = max(
                abs(final[-2][0] - final[-3][0]), abs(final[-2][1] - final[-3][1])
            )
...
def route_astar(
    ...
    **kwargs: Any,
    ...
    grid_static, x, y = _generate_grid(
        component, resolution, all_avoid_layers, distance
    )
    ...
    if avoid_same_layer and routing_layer is not None:
        _mark_layer_obstacles(
            grid_static, component, routing_layer, x_vals, y_vals, resolution, distance
        )
    ...
    for route_idx, (p1, p2) in enumerate(zip(port1, port2)):
        ...
        cleaned_waypoints = _simplify_manhattan_path(
            final_waypoints, min_segment_length
        )
