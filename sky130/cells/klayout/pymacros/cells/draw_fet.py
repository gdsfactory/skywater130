...
def draw_nfet(
    ...
    sd_l_con = ((sd_con_col)*licon_size[0]) + ((sd_con_col-1)*licon_spacing[0]) + 2*licon_dt_enc[0]
    sd_l = sd_l_con + 0.05
    ...
    l_d = sd_l + nf*l + (nf-1)*inter_sd_l +  sd_l #  diffution total length 
    ...
    if nf == 1 :
        poly = c_inst.add_ref(gf.components.rectangle(size=(l,w+2*end_cap),layer=poly_layer))
        ...
        rect_p1 = gf.components.rectangle(size = (l,w_p1), layer= poly_layer)
        rect_p2 = gf.components.rectangle(size = (l,w_p2), layer= poly_layer)
...
def draw_pfet(
    ...
    if bulk == "None":
        ...
        , layer= nsdm_layer))
        nsdm_in.move((rect_bulk_in.xmin + tap_nsdm_enc, rect_bulk_in.ymin + tap_nsdm_enc))
        ...
        # generating nwell 
        nwell = c.add_ref(gf.components.rectangle(size=(nsdm_out.xmax-nsdm_out.xmin + 2*nwell_enclosing, nsdm_out.ymax-nsdm_out.ymin + 2*nwell_enclosing)
        , layer=nwell_layer))
    ...
    if bulk != "Gaurd Ring":
        ...
        # nwell generation 
        nwell = c.add_ref(gf.components.rectangle(size=(c_inst.xmax - c_inst.xmin + 2*nwell_enclosing
        , (c_inst.ymax-c_inst.ymin)  + 2*nwell_enclosing),layer=nwell_layer))
    ...
    if type == "sky130_fd_pr__pfet_01v8_hvt":
        hvt = c_inst.add_ref(gf.components.rectangle(size= (nf*l + (nf-1)*inter_sd_l + 2*gate_hvt_enc, w + 2*gate_hvt_enc)
        , layer= hvtp_layer))
        hvt.move((sd_l - gate_hvt_enc,-gate_hvt_enc))

    if type == "sky130_fd_pr__pfet_g5v0d10v5" :
        hvi = c.add_ref(gf.components.rectangle(size=(nwell.xmax-nwell.xmin,nwell.ymax-nwell.ymin),layer=hvi_layer))
