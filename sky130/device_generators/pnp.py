...
def pnp(
    ...
    for i in range(2) : 
        ...
        rect_out = gf.components.rectangle(size = (E_width + 2*np_spacing + 2*B_width - ((B_width - nc_v*contact_size[0] - (nc_v-1)*contact_spacing[0]) )+ 2*i*mcon_enclosure[0] + 2*(1-i)*li_enclosure,
        E_length + 2*np_spacing + 2*B_width - ((B_width - nr_h*contact_size[1] - (nr_h-1)*contact_spacing[1])) + 2*i*mcon_enclosure[1] + 2*(1-i)*li_enclosure), layer= rect_layer[i])
    
        li_m1_b_in = c_B.add_ref(rect_in)
        li_m1_b_out = c_B.add_ref(rect_out)

        li_m1_b_in.connect("e1", destination= E.ports["e1"])
        li_m1_b_in .movex((E_width + np_spacing + (B_width - nc_v*contact_size[0] - (nc_v-1)*contact_spacing[0])/2 )- i*mcon_enclosure[0] - (1-i)*li_enclosure)

        li_m1_b_out.connect("e1", destination= E.ports["e1"])
        li_m1_b_out .movex((E_width + np_spacing+ B_width - ((B_width - nc_v*contact_size[0] - (nc_v-1)*contact_spacing[0])/2) + i*mcon_enclosure[0]) + (1-i)*li_enclosure )
    ...
    for i in range(2):
        ...
        rect_in = gf.components.rectangle(size = (E_width + 4.5*np_spacing + 2*B_width + (C_width - nc_v*contact_size[0] - (nc_v-1)*contact_spacing[0]) - 2*i*mcon_enclosure[0] - 2*(1-i)*li_enclosure,
        E_length + 4.5*np_spacing + 2*B_width  + (C_width - nr_h*contact_size[1] - (nr_h-1)*contact_spacing[1]) - 2*i*mcon_enclosure[1] - 2*(1-i)*li_enclosure), layer= rect_layer[i])
   
        rect_out = gf.components.rectangle(size = (E_width + 4.5*np_spacing + 2*B_width + 2*C_width - (C_width - nc_v*contact_size[0] - (nc_v-1)*contact_spacing[0]) + 2*i*mcon_enclosure[0] + 2*(1-i)*li_enclosure, 
        E_length + 4.5*np_spacing + 2*B_width + 2*C_width - (C_width - nr_h*contact_size[0] - (nr_h-1)*contact_spacing[0]) + 2*i*mcon_enclosure[1]+ 2*(1-i)*li_enclosure), layer= rect_layer[i])
    
        li_m1_c_in = c_C.add_ref(rect_in)
        li_m1_c_out = c_C.add_ref(rect_out)

        li_m1_c_in.connect("e1", destination= E.ports["e1"])
        li_m1_c_in .movex(E_width + 2.25*np_spacing + B_width + (C_width - nc_v*contact_size[0] - (nc_v-1)*contact_spacing[0])/2 - i*mcon_enclosure[0] - (1-i)*li_enclosure)

        li_m1_c_out.connect("e1", destination= E.ports["e1"])
        li_m1_c_out .movex(E_width + 2.25*np_spacing+ B_width + C_width - (C_width - nc_v*contact_size[0] - (nc_v-1)*contact_spacing[0])/2 + i*mcon_enclosure[0] + (1-i)*li_enclosure)
...
if __name__ == "__main__":
    ...
    c = pnp(E_length=3.4, E_width=3.4, np_spacing=1)
