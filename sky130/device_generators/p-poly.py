...
def p_poly ( 
    ...
    rect_r = gf.components.rectangle(size= (p_poly_width,p_poly_length), layer= poly_res_layer)
    R_1 = c.add_ref(rect_r) 

    # generate polysilicon R0

    p_length = (licon_slots_size[1]+ 2*contact_enclosure[1])
    rect_p = gf.components.rectangle(size= (p_poly_width, p_poly_length + 2*p_length), layer= poly_layer)
    ...
    if (p_poly_width - nc*licon_slots_size[0] - (nc-1)*licon_slots_spacing[0])/2 < contact_enclosure[0] :
        nc-=1 
    ...
    # generate li (local interconnects)

    rect_li = gf.components.rectangle(size= (nc*licon_slots_size[0] + (nc-1)*licon_slots_spacing[0] + 2*li_enclosure 
    ,licon_slots_size[1] + 2*li_enclosure), layer= li_layer)

    li = c.add_array(rect_li, rows=2 , columns= 1 , spacing= (0, p_poly_length + 2* contact_enclosure[1] + licon_slots_size[1]))
    li.movey(- licon_slots_size[1] - contact_enclosure[1]- li_enclosure)
    li.movex((p_poly_width - nc*licon_slots_size[0] - (nc-1)*licon_slots_spacing[0] - 2*li_enclosure)/2)

    # generate mcon 

    rect_mc = gf.components.rectangle(size= contact_size, layer= mcon_layer)

    nr_m = ceil ((rect_li.ymax - rect_li.ymin)/(contact_size[1]+ contact_spacing[1]))
    if (rect_li.ymax - rect_li.ymin - nr_m*contact_size[1]- (nr_m-1)*contact_spacing[1])/2 < contact_enclosure[1] :
        nr_m -= 1
    
    nc_m = ceil ((rect_li.xmax - rect_li.xmin)/ (contact_size[0]+contact_spacing[0]))
    if (rect_li.xmax - rect_li.xmin - nc_m*contact_size[0] - (nc_m-1)*contact_spacing[0]) < contact_enclosure[0] :
        nc_m -= 1
    ...
    for i in range(2):
        ...
        mcon_arr.movey((1-i)*(- licon_slots_size[1] - contact_enclosure[1]- li_enclosure) + i*(p_poly_length ))
        mcon_arr.movex((rect_li.xmax - rect_li.xmin - nc_m*contact_size[0] - (nc_m-1)*contact_spacing[0])/2)
        mcon_arr.movey((rect_li.ymax - rect_li.ymin - nr_m*contact_size[1] - (nr_m-1)*contact_spacing[1])/2)


    # generate npc (nitride poly cut)

    rect_npc = gf.components.rectangle(size= (p_poly_width+ 2*npc_enclosure[0], p_poly_length + 2*p_length + 2*npc_enclosure[1]), layer= npc_layer)
